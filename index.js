...
function replaceSpecialCharacters(path) {
    return path.replace(/\+/g, '%2B').replace(/\[/g, '%5B').replace(/\]/g, '%5D');
}
...
export default {
    async fetch(request, env) {
        ...
        // Log the replaced path for debugging
        console.log(`Replaced path: ${path}`);

        const pathSegments = path.split('/');

        if (env['ALLOW_LIST_BUCKET'] !== "true") {
            if ((env['BUCKET_NAME'] === "$path" && pathSegments.length < 2)
        ...
        url.pathname = `/${path}`;  // Ensure the replaced path is used in the URL
